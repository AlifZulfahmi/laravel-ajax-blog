        ...
        if ($request->ajax()) {
            ...
            if (empty($request->search['value'])) {
                $data = User::offset($start)
                    ->limit($limit)
                    ->get(['id', 'name', 'email', 'created_at', 'is_verified', 'role']);
            } else {
            ...
            return DataTables::of($data)
                ...
                ->editColumn('role', function ($data) {
                    return "<span class='badge " . ($data->role === 'writer' ? 'bg-primary' : 'bg-secondary') . "'>" . ucfirst($data->role) . "</span>";
                })
                ...
                ->rawColumns(['action', 'is_verified', 'role'])
                ->make();
        }
    }
